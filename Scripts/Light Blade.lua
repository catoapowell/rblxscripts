...
Part=part(Enum.FormFactor.Custom,m,Enum.Material.SmoothPlastic,0,0,"Really black","SwordPart",Vector3.new(1.26045096, 0.762464345, 0.762464285))
...
Hitbox=part(Enum.FormFactor.Custom,m,Enum.Material.SmoothPlastic,0,1,"White","Hitbox",Vector3.new(0.650000036, 4.19999981, 0.200000003))
...
EffectPart=part(Enum.FormFactor.Custom,m,Enum.Material.SmoothPlastic,0,1,"Institutional white","EffectPart",Vector3.new(0.200000003, 0.200000003, 0.200000003))
...
effectsg.BrickColor = BrickColor.new("Institutional white")
...
coroutine.resume(coroutine.create(function()
	...
	for i=0,200 do
	wait()
	DerpMagic(Part,1,i/3,1,0,i/3,0,BrickColor.new("Institutional white")) 
	...
	for i=0,100 do
	wait()
	DerpMagic(Part,1,fff/3,1,0,fff/3,0,BrickColor.new("Institutional white")) 
	Part.CFrame=cframe*CFrame.fromEulerAnglesXYZ(math.random(-50,50),math.random(-50,50),math.random(-50,50))
	end
	for i=0,1,0.05 do
	wait()
...
MagicBlock(BrickColor.new("White"),parent.CFrame,4,4,4,0.2,0.2,0.2,0.01)
MagicWaveThing(BrickColor.new("White"),parent.CFrame,4,4,4,0.2,0.2,0.2,0.01)
end
...
if size==3 then
MagicRing(BrickColor.new("White"),parent.CFrame,.5,.5,.5,1,1,1,0.1,3,parent)
elseif size==2 then
MagicRing(BrickColor.new("White"),parent.CFrame,.5,.5,.5,1,1,1,0.1,3,parent)
end
end
if counter%5==0 then
if size==1 then
MagicBlock(BrickColor.new("White"),parent.CFrame,.5,.5,.5,1,1,1,0.1,3,parent)
elseif size==2 then
MagicBlock(BrickColor.new("White"),parent.CFrame,.5,.5,.5,1,1,1,0.1,3,parent)
elseif size==3 then
MagicBlock(BrickColor.new("White"),parent.CFrame,.5,.5,.5,1,1,1,0.1,3,parent)
...
local con=Hitbox.Touched:connect(function(hit) Damagefunc(Hitbox,hit,4,11,math.random(5,5),"Normal",RootPart,.2,1) end) 
...
attack=true
local hitsounds={"199149137","199149186","199149221","199149235","199149269","199149297"}
local con=Hitbox.Touched:connect(function(hit) Damagefunc(Hitbox,hit,6,15,math.random(5,5),"Normal",RootPart,.2,1) end) 
local fx=Hitbox.Touched:connect(function(part)
	local human=part.Parent:findFirstChild("Humanoid")
	if human~=nil and bounce==false then
		bounce=true
...
attack = true
local hitsounds={"199149137","199149186","199149221","199149235","199149269","199149297"}
local con=Hitbox.Touched:connect(function(hit) Damagefunc(Hitbox,hit,9,18,math.random(20,30),"Normal",RootPart,.2,1) end)
local fx=Hitbox.Touched:connect(function(part)
	local human=part.Parent:findFirstChild("Humanoid")
	if human~=nil and bounce==false then
		bounce=true
...
local function boom()
local pprt=ppart(3,game.Workspace,0,1,BrickColor.new("White"),"Effect",vt(1,1,1))
pprt.Anchored=true
...
local wave=part(3,game.Workspace,"Neon",0,.7,BrickColor.new("White"),"Effect",vt(1,1,1))
...
MagniDamage(wave,20,15,25,math.random(20,30),"Knockdown",RootPart)
for i=0,80 do
swait()
MagicBlock(BrickColor.new("White"),EffectPart.CFrame,7,7,7,-0.5,-0.5,-0.5,0.05)
MagicCircle(BrickColor.new("White"),EffectPart.CFrame*euler(math.random(-50,50),math.random(-50,50),math.random(-50,50))*cf(0,5,0),0.5,1,0.5,0.02,1,0.02,0.05)
...
efprt=part(Enum.FormFactor.Custom,game.Workspace,Enum.Material.Neon,0,0,"White","BallEffect",Vector3.new(2, 2, 2))
...
MagniDamage(efprt,15,15,25,math.random(20,30),"Knockdown",RootPart)
...
delay(0.5,function ()
MagniDamage(Torso,2000,35,50,math.random(50,70),"Knockdown",RootPart)
end)
Humanoid.Health=Humanoid.Health-75
...
function SolSiderea()
	...
	local n=2
	local orb=ppart(3,Character,0,1,BrickColor.new("Institutional white"),"Orb",vt())
	...
	MagicBlock(BrickColor.new("Institutional white"),orb.CFrame,15,15,15,2,2,2,0.1)
	local ef=ppart(3,workspace,0,0,BrickColor.new("Institutional white"),"Effect",vt())
	ef.Anchored=true
	...
	MagicCylinder(BrickColor.new("Institutional white"),CFrame.new((orb.Position+pos)/2,pos)*euler(1.57,0,0),10,mag*5,10,0.5,0,0.5,0.01)
	for i=1,2 do
	end
	for i=1,5 do
	...
	local ref=ppart(3,workspace,0,1,BrickColor.new("Institutional white"),"Effect",vt())
	ref.CFrame=cf(pos)
	coroutine.resume(coroutine.create(function(Part) 
	wait(1)
	Part.Parent=nil
	end),ref)
	MagniDamage(ref,25,20,30,40,"Knockdown",ref)
	if hit~=nil then
	Damagefunc(ref,hit,50,70,60,"Knockdown",RootPart,0)
	end
	local n=2
	MagicWave(BrickColor.new("Institutional white"),cf(pos)*euler(0,math.random(-50,50),0),2,2,2,.5,.3,.5,0.01)
	swait(75)
	Humanoid.WalkSpeed=16
	attack=false
end
